...
std::map<Calcer::ErrorType, std::string>Calcer::ErrorType2Name = {
...
const TokenLevel Calcer::tokenLevel{
        {'+', 1},{'-', 1},
        {'*', 2},{'/', 2},{'^', 2},{'%', 2},{'\\', 2},
        {'(', 0},{')', 0}
};

const std::map<std::string, int> Calcer::NoteTable = {
    {"+",2},{"-",2},
    {"*",2},{"/",2},{"^", 2},{"%", 2},{"\\", 2},
    {"sqrt",1}, {"abs",1},
    {"sin",1},{"cos", 1}, {"tan", 1},
...
Calcer::Calcer(QObject *parent) : QObject(parent)
{

}

std::string Calcer::calc(std::string expression)
{
    Error e;
    const auto res = reversePolishNotation(expression, tokenLevel, e);
    if (e.type != ErrorType::Well) {
        return "[" + ErrorType2Name[e.type] + "]: " + e.msg + ".";
    }
...
Notation Calcer::reversePolishNotation(const std::string &expression, const TokenLevel &tokenLevel, Calcer::Error &e)
{
    Notation result;
...
bool Calcer::checkNumber(std::string::const_iterator &it, const std::string &expression)
{
    return isdigit(*it)
...
std::string Calcer::readNumber(std::string::const_iterator &it, const std::string &expression)
{
    std::string rtn;
...
bool Calcer::checkFunc(std::string::const_iterator &it, const std::string &expression)
{
    return isalnum(*it);
}

std::string Calcer::readFunc(std::string::const_iterator &it, const std::string &expression)
{
    std::string rtn;
...
double Calcer::evalNotation(const Notation &notation, Calcer::Error &e)
{
